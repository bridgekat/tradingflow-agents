...
| `src/features.rs` | The factor catalog and the model-ready `(N, F)` panel. **A new factor is one `add("NAME", handle)` line**; the seed catalog demonstrates one factor per idiom (rolling price/return stats, size, balance-sheet and TTM-flow valuation, liquidity) of the 100+-factor handbook catalogs. |

...
| `src/features/` | The factor catalogs and the model-ready `(N, F)` panel, selected by `--features`. `basic.rs` is hand-wired: **a new factor is one `add("NAME", handle)` line**, and its seed catalog demonstrates one factor per idiom (rolling price/return stats, size, balance-sheet and TTM-flow valuation, liquidity). `alpha101.rs`, `alpha158.rs`, `alpha360.rs` and `cicc_pv.rs` are ported catalogs: **a new factor there is one formula string**, lowered through a shared `expr::Context` (plus two custom window reductions in `cicc_pv.rs`); `cicc_fund.rs` is the CICC fundamental handbook, hand-wired like `basic.rs`. |
